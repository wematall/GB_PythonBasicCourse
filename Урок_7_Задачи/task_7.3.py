...
class Cell:
  ...
  def __add__(self, newcell):
    return self.amount + newcell.amount

  def __sub__(self, newcell):
    if self.amount - newcell.amount > 0:
      return self.amount - newcell.amount
    else:
      return "Вычитание невозможно"

  def __mul__(self, newcell):
    return self.amount * newcell.amount

  def __truediv__(self, newcell):
    return round(self.amount // newcell.amount)

  def make_order(self, row_size):
    row = ''
    for i in range(self.amount // row_size):
      row += f'{"*" * row_size}\\n'
    row += f'{"*" * (self.amount % row_size)}'
    return row
